...
ALTER TABLE clean_certificates
ADD PROPERTY_TYPE NVARCHAR(255);

UPDATE c
SET c.PROPERTY_TYPE = s.PROPERTY_TYPE
FROM clean_certificates c
JOIN dbo.certificates s
    ON c.LMK_KEY = s.LMK_KEY;

	select*
	from clean_certificates;

SELECT TOP 20 LMK_KEY, PROPERTY_TYPE
FROM clean_certificates
